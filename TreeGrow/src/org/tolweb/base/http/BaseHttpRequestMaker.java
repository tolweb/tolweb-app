...
public class BaseHttpRequestMaker {
    ...
    public static String getExternalUrlString(String pageName, Map additionalArgs, String prefix, String userId, String password) {
        ...
        if (additionalArgs != null) {
	        if (StringUtils.notEmpty(userId)) {
		        if (additionalArgs.get(RequestParameters.USER_ID) == null) {
		            returnString = addParameter(returnString, RequestParameters.USER_ID, userId);
		        }
		        if (additionalArgs.get(RequestParameters.PASSWORD) == null) {
		            returnString = addParameter(returnString, RequestParameters.PASSWORD, password);
		        }
	        }
    ...
    protected static String addParameter(String originalString, String paramName, String value) {
    	if (StringUtils.notEmpty(originalString)) {
	        ...
	        originalString += "=";
	        try {
				originalString += URLEncoder.encode(value, "UTF-8");
    ...
    public static Document getDocumentFromBytes(byte[] bytes) {
        Document doc;
        try {
            doc = new SAXBuilder().build(new ByteArrayInputStream(bytes));
            return doc;
        } catch (Exception e) {
