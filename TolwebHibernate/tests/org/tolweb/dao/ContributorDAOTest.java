...
java.io.PrintWriter w =
    ...
    public void testSaveStudent() {
    	dao.addContributor(getNewStudentInstance("alias1"), editHistoryDao);
    	Student sameStudent = dao.getStudentWithAlias("alias1");
    	assertNotNull(sameStudent);
    	assertEquals("alias1", sameStudent.getAlias());
    	assertEquals("beetle@ag.arizona.edu", sameStudent.getTeacher().getEmail());
    }
    
    public void testGetProjectsForTeacher() {
    	Contributor contr = dao.getContributorWithEmail("beetle@ag.arizona.edu");

    	ClassroomProject project1 = new ClassroomProject();
    	project1.setClassGroup("1st period");
    	project1.setDescription("this is some interesting project");
    	project1.setName("cool project");
    	project1.setTeacher(contr);
    	Student stud1 = getNewStudentInstance("alias1");
    	Student stud2 = getNewStudentInstance("alias2");
    	dao.addContributor(stud1, editHistoryDao);
    	dao.addContributor(stud2, editHistoryDao);
    	project1.addToStudents(stud1);
    	project1.addToStudents(stud2);
    	dao.addProject(project1, editHistoryDao);
    	
    	List projects = dao.getProjectsForContributor(contr);
    	ClassroomProject otherProject = (ClassroomProject) projects.get(0);
    	assertEquals(otherProject.getClassGroup(), "1st period");
    	assertEquals(otherProject.getDescription(), "this is some interesting project");
    	assertEquals(otherProject.getName(), "cool project");
    	assertEquals(otherProject.getStudents().size(), 2);
    }
    
    private Student getNewStudentInstance(String alias) {
    	Student newStudent = new Student();
    	newStudent.setEmail("");
    	newStudent.setFirstName("");
    	newStudent.setLastName("");
    	newStudent.setPassword("");
    	newStudent.setAlias(alias);
    	newStudent.setTeacher(dao.getContributorWithEmail("beetle@ag.arizona.edu"));
    	return newStudent;
    }
