...
public class AccessoryPageDAOTest extends ApplicationContextTestAbstract {
    ...
    public AccessoryPageDAOTest(String name) {
        super(name);
        dao = (AccessoryPageDAO) context.getBean("accessoryPageDAO");
        publicDao = (AccessoryPageDAO) context
                .getBean("publicAccessoryPageDAO");
    ...
    public void testFindAccessoryPage() {
        ...
        AccessoryPageContributor contr = (AccessoryPageContributor) contributors
                .iterator().next();
    ...
    public void testQuerying() {
        ...
        results = dao.getAccessoryPagesMatchingCriteria(args);
        assertEquals(results.size(), 1);
        AccessoryPageContributor contr = (AccessoryPageContributor) ((MappedAccessoryPage) results
                .get(0)).getContributors().iterator().next();
    ...
    public void testFieldsMatching() {
        ...
        accPage.setSubmittedContributor(contributorDao
                .getContributorWithId(submissionContributorId.toString()));
        ...
        MappedNode euks = (MappedNode) nodeDao.findNodesExactlyNamed(
                "Eukaryotes").get(0);
    ...
    public void testSubmittedPages() {
        List list = dao.getSubmittedTreehouses();
    ...
    public void testPushing(MappedAccessoryPage pageToPush) {
        ...
        MappedAccessoryPage publicPage = publicDao
                .getAccessoryPageWithId(pageToPush.getAccessoryPageId());
        verifyValuesOnPages(pageToPush, publicPage);
    }

    @SuppressWarnings("unchecked")
    private void verifyValuesOnPages(MappedAccessoryPage foundPage,
            MappedAccessoryPage accPage) {
        assertEquals(foundPage.getTreehouseType(), accPage.getTreehouseType());
        assertEquals(foundPage.getNotes(), accPage.getNotes());
        assertEquals(foundPage.getAcknowledgements(), accPage
                .getAcknowledgements());
        ...
        assertEquals(foundPage.getContributors().size(), accPage
        ...
        AccessoryPageContributor c1 = (AccessoryPageContributor) foundPage
                .getContributors().iterator().next();
        AccessoryPageContributor c2 = (AccessoryPageContributor) accPage
                .getContributors().iterator().next();
        assertEquals(c1.getContributorId(), c2.getContributorId());
        assertEquals(c1.getIsAuthor(), c2.getIsAuthor());

        assertEquals(foundPage.getInternetLinks().size(), accPage
                .getInternetLinks().size());
        ...
        //assertEquals(foundPage.getPrimaryAttachedNode().getNodeId(),
        // accPage.getPrimaryAttachedNode().getNodeId());
        ...
        assertEquals(foundPage.getIsAdvancedLevel(), accPage
                .getIsAdvancedLevel());
        assertEquals(foundPage.getIsArtAndCulture(), accPage
                .getIsArtAndCulture());
        assertEquals(foundPage.getIsBeginnerLevel(), accPage
                .getIsBeginnerLevel());
        assertEquals(foundPage.getIsGame(), accPage.getIsGame());
        assertEquals(foundPage.getIsIntermediateLevel(), accPage
                .getIsIntermediateLevel());
        assertEquals(foundPage.getIsInvestigation(), accPage
                .getIsInvestigation());
        assertEquals(foundPage.getIsStory(), accPage.getIsStory());
        assertEquals(foundPage.getIsTeacherResource(), accPage
                .getIsTeacherResource());
        assertEquals(foundPage.getIsBiography(), accPage.getIsBiography());
        assertEquals(foundPage.getLastEditedContributorId(), accPage
                .getLastEditedContributorId());
        assertEquals(foundPage.getSubmittedContributorId(), accPage
                .getSubmittedContributorId());
        ...
        if (foundPage.getSubmittedContributor() != null) {
            assertEquals(foundPage.getSubmittedContributor().getName(), accPage
                    .getSubmittedContributor().getName());
        }
    }
    ...
    public void testNodeOrder() {
        ...
        MappedAccessoryPage savedPg = dao.getAccessoryPageWithId(pg
                .getAccessoryPageId());
    ...
    private SortedSet constructLifeNodesSet() {
        ...
        MappedNode result = (MappedNode) nodeDao.findNodesExactlyNamed("Life")
                .get(0);
        AccessoryPageNode node = new AccessoryPageNode();
        node.setNode(result);
    ...
    private SortedSet constructMoreThanLifeNodesSet() {
        ...
        euks.setNode((MappedNode) nodeDao.findNodesExactlyNamed("Eukaryotes")
                .get(0));
        euks.setIsPrimaryAttachedNode(false);
        euks.setShowLink(true);
        nodes.add(euks);
        AccessoryPageNode viruses = new AccessoryPageNode();
        viruses.setNode((MappedNode) nodeDao.findNodesExactlyNamed("Viruses")
                .get(0));
